...
import {
  CreateFarmsInterface,
  CreateFarmsItems,
  CreateGroupInterface,
  CreateGroupItems,
  ReportByFarmInterface,
  ReportByFarmItems,
} from './Interfaces';
...
export const updateGroupData = async (
  ...
  const itemIndex = doc.items.findIndex(
    (item: CreateGroupItems) => item.group_id === isSelectedGroup.group_id
  );
...
export const deleteGroupData = async (
  ...
  if (itemIndex >= 0) {
    ...
    if (farmDoc?.items?.length) {
      farmDoc.items = farmDoc.items.filter(
        (item: ReportByFarmItems) => item.group_id !== isSelectedGroup.group_id
      );
    ...
    if (reportDoc?.items) {
      reportDoc.items = reportDoc.items.filter(
    ...
    console.log(
      `Item with id 'item2' not found in document '${GroupListDoc}'.`
    );
  }
};
...
export const updateFarmData = async (
  db: any,
  newItem: CreateFarmsItems,
  isSelectedFarmItem: CreateFarmsItems
) => {
  // Try to retrieve the document by ID
  const doc = await db.get(GroupByFarmListDoc);
  const itemIndex = doc.items.findIndex(
    (item: CreateFarmsItems) => item.farm_id === isSelectedFarmItem.farm_id
  );
  // Update the item with the new data
  doc.items[itemIndex] = {
    ...isSelectedFarmItem,
...
export const deleteFarmData = async (
  db: any,
  isSelectedFarmItem: CreateFarmsItems
) => {
  // Get the document by its ID
...
export const fetchAllFarmByGroupList = async (db: any, groupId: number) => {
  ...
  if (groupRes?.items?.length) {
    const filteredData = groupRes.items.filter(
      (item: CreateFarmsItems) => item.group_id === groupId
    );
    return filteredData;
  }
};
...
export const fetchAllReportDataByFarm = async (
  db: any,
  groupId: number,
  farm_id: number,
  toDate?: string,
  ...
  const reportRes = await db.get(ReportByFarm).catch((err: any) => {
    console.log('err ', err);
  });
  if (reportRes?.items?.length) {
    const to_Date = toDate
      ? new Date(toDate)
      : new Date(endDateFormatToUTC(new Date().toString()));
    ...
    const filteredData = reportRes.items.filter((item: ReportByFarmItems) => {
      if (toDate?.length && fromDate?.length) {
        if (
          ...
          (new Date(item.create_time) >= to_Date ||
            new Date(item.create_time) === to_Date) &&
        ...
        if (item.group_id === groupId && item.farm_id === farm_id) {
          return item;
        }
      }
    });
...
export const fetchAllReportDataByGroup = async (
  db: any,
  groupId: number,
  toDate: string,
  fromDate?: string
...
export const fetchAllReportDataByDate = async (
  db: any,
  toDate: string,
  fromDate?: string
) => {
...
export const fetchSamplesCountByFarm = async (
  db: any,
  groupId: number,
  farm_id: number
) => {
